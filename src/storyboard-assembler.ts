 ...
 */
export function assembleImages(markdown: string, sceneImages: SceneImageMap): string {
	...
	for (let i = 0; i < lines.length; i++) {
		...
		if (currentSceneNumber !== null && (sceneMatch || line.trim() === "---")) {
			const entry = sceneImages[`scene-${currentSceneNumber}`] ?? sceneImages[currentSceneNumber];
...
function renderCharacter(character: Character): string {
	...
	if (character.sheet) {
		lines.push(`![${character.slug}-character-sheet](${character.sheet.imageUrl})`);
		lines.push("");
