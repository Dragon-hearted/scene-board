...
## Summary
SceneBoard transforms video briefs of any format into a professional **two-phase storyboard deliverable** through a dynamic, approval-gated workflow: a **Phase 1 composite multi-panel storyboard sheet image** (one per ≤15s block, generated via the ImageEngine HTTP service using GPT Image 2) plus a **Phase 2 cinematic video prompt**. It serves both the internal creative team and the client-facing team, leveraging marketing, sales, social media, and ads skills.
...
4. **Scene/Panel Breakdown** — Expand the approved structure into numbered panels, each a short shot of ≤2s (defaulting to ~1s), with per-panel timecodes (≤15s per sheet).
5. **Visual Direction** — Lock a Style Anchor and define per-panel visual direction.
6. **Reference Sheet stage (4.5, optional)** — Generate 4-view character/product reference sheets (via ImageEngine), routed by `brand_category`.
...
## Key Source Files
- `src/image-client.ts` — ImageEngine HTTP client (the sole image transport)
- `src/image-provider.ts` — provider façade (ImageEngine-only; omits `model` to use ImageEngine's default GPT Image 2 provider, then downloads the result to `outPath`)
